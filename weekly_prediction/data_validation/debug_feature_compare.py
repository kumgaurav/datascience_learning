...
def _normalize_prices(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Drop duplicate columns after case-normalization (keep first occurrence)
    try:
...
def main() -> None:
    ...
    # Extend window backward to satisfy rolling windows (RSI 14d, Vol MA 20d)
    pad_days = 25
    pre_start = start_dt - pd.Timedelta(days=pad_days)
    a = prices[(prices['ticker'] == args.ticker_a.upper()) & (prices['date'].between(pre_start, end_dt))].copy()
    b = prices[(prices['ticker'] == args.ticker_b.upper()) & (prices['date'].between(pre_start, end_dt))].copy()
    ...
    # Restrict to requested analysis window for display
    a_win = a_feat[a_feat['date'].between(start_dt, end_dt)]
    b_win = b_feat[b_feat['date'].between(start_dt, end_dt)]

    # Print last 3 rows (most recent)
    print(f"\n=== Most recent 3 days (ending {end_dt.date()}) ===")
    recent = _side_by_side(a_win.tail(3), b_win.tail(3), cols=('open','high','low','close','volume'))
    print(recent.to_string(index=False))

    # Print earliest 3 rows (start of window)
    print(f"\n=== Earliest 3 days in window (starting {start_dt.date()}) ===")
    earliest = _side_by_side(a_win.head(3), b_win.head(3), cols=('open','high','low','close','volume'))
    print(earliest.to_string(index=False))

    # Show RSI 14d and Volume Ratio series tails
    print("\n=== RSI(14) tails ===")
    print(pd.DataFrame({
        'date': a_win['date'].tail(5).dt.date,
        f'RSI_14_{args.ticker_a.upper()}': a_win['rsi_14d'].tail(5).round(3).values,
        f'RSI_14_{args.ticker_b.upper()}': b_win['rsi_14d'].tail(5).round(3).values,
    }).to_string(index=False))

    print("\n=== Volume Ratio tails (vol/vol_ma_20) ===")
    print(pd.DataFrame({
        'date': a_win['date'].tail(5).dt.date,
        f'VolRatio_{args.ticker_a.upper()}': a_win['volume_ratio'].tail(5).round(3).values,
        f'VolRatio_{args.ticker_b.upper()}': b_win['volume_ratio'].tail(5).round(3).values,
    }).to_string(index=False))
    ...
    print(f"RSI_14 {args.ticker_a.upper()} = {float(a_win['rsi_14d'].tail(1).iloc[0]):.3f} (window start {_dstr(a_rsi_start)})")
    print(f"RSI_14 {args.ticker_b.upper()} = {float(b_win['rsi_14d'].tail(1).iloc[0]):.3f} (window start {_dstr(b_rsi_start)})")
    print(f"VolRatio {args.ticker_a.upper()} = {float(a_win['volume_ratio'].tail(1).iloc[0]):.3f} (vol_ma_20 from last 20 days starting {_dstr(a_vol_start)})")
    print(f"VolRatio {args.ticker_b.upper()} = {float(b_win['volume_ratio'].tail(1).iloc[0]):.3f} (vol_ma_20 from last 20 days starting {_dstr(b_vol_start)})")
