...
# Load config and apply env (so paths are overridable via YAML)
try:
    ...
    _CFG_PATH = os.getenv('APP_CONFIG', os.path.join(os.path.dirname(__file__), 'snapshot_config.yaml'))
    _CFG = load_config(_CFG_PATH)
    apply_env_from_config(_CFG)
except Exception:
    _CFG = {}
...
def find_latest_snapshot_dir(base_dir: str = None) -> Optional[str]:
    try:
        base = base_dir or os.getenv('SNAPSHOT_ROOT_DIR', 'snapshot')
...
def _load_ensemble_pred_map(cfg: dict) -> Optional[dict]:
    """Load ensemble predictions from config-defined CSV and return {ticker: predicted_value}.

    Preference order of prediction columns: ensemble_score > lstm_pred > lstm_predicted_return_pct > pred_lstm > xgb_pred > pred_xgb.
    """
    try:
        paths = cfg.get('paths') if isinstance(cfg, dict) else {}
        cand_paths: List[str] = []
        if isinstance(paths, dict):
            for key in ['predictions_csv', 'ensemble_out', 'xgb_ranked_output_csv', 'xgb_weekly_output_csv']:
                p = paths.get(key)
                if isinstance(p, str):
                    cand_paths.append(p)
        # Fallback defaults
        cand_paths += [
            'data/ensemble/ensemble_weekly_output.csv',
            'data/top/ensemble_scores_output.csv',
        ]
        # Pick first existing
        csv_path = next((p for p in cand_paths if isinstance(p, str) and os.path.exists(p)), None)
        if not csv_path:
            return None
        ...
        col_order = ['ensemble_score', 'lstm_pred', 'lstm_predicted_return_pct', 'pred_lstm', 'xgb_pred', 'pred_xgb']
        pred_col = next((c for c in col_order if c in df.columns), None)
        if not pred_col:
            return None
        # Numeric map
        out = {}
        vals = pd.to_numeric(df[pred_col], errors='coerce')
        for t, v in zip(df['ticker'], vals):
            if pd.notna(v):
                out[str(t).upper()] = float(v)
        return out if out else None
    except Exception:
        return None


def _load_ensemble_ui_order(cfg: dict, top_n: int = 25) -> Optional[List[str]]:
    """Load top-N tickers ordered by ensemble score from configured predictions CSVs."""
    try:
        paths = cfg.get('paths') if isinstance(cfg, dict) else {}
        cand_paths: List[str] = []
        if isinstance(paths, dict):
            for key in ['predictions_csv', 'ensemble_out']:
                p = paths.get(key)
                if isinstance(p, str):
                    cand_paths.append(p)
        cand_paths += [
        ...
        path = next((p for p in cand_paths if isinstance(p, str) and os.path.exists(p)), None)
        if not path:
            return None
        df = pd.read_csv(path)
        if 'ticker' not in df.columns:
            return None
        df['ticker'] = df['ticker'].astype(str).str.upper()
        # Prefer ensemble_score for ordering
        order_col = 'ensemble_score' if 'ensemble_score' in df.columns else None
        if order_col:
            df = df.sort_values(order_col, ascending=False)
        return df['ticker'].drop_duplicates().head(top_n).tolist()
    except Exception:
        return None


def _load_xgb_ui_order(cfg: dict, top_n: int = 25) -> Optional[List[str]]:
    """Load the first N tickers from the XGB weekly output CSV specified in config (or fallbacks)."""
    try:
        paths = cfg.get('paths') if isinstance(cfg, dict) else {}
        cand_paths: List[str] = []
        if isinstance(paths, dict):
            p = paths.get('xgb_weekly_output_csv')
            if isinstance(p, str):
                cand_paths.append(p)
        cand_paths += [
            'data/xgboost/xgb_weekly_output.csv',
        ...
        if 'ticker' not in df.columns:
            return None
        df['ticker'] = df['ticker'].astype(str).str.upper()
        return df['ticker'].head(top_n).dropna().astype(str).str.upper().tolist()
    except Exception:
        return None
...
def render_outcomes_tab(snapshot_dir: str) -> None:
    ...
    # Establish UI order per section
    ui_order_xgb = _load_xgb_ui_order(_CFG, top_n=25) or load_ui_order([
        _CFG.get('paths', {}).get('xgb_ranked_csv', ''),
        _CFG.get('paths', {}).get('xgb_ranked_output_csv', ''),
        'data/top/xgb_ranked.csv',
        'data/top/xgb_ranked_output.csv',
    ])
    ui_order_ens = _load_ensemble_ui_order(_CFG, top_n=25) or ui_order_xgb
    if not ui_order_xgb:
        st.warning("Could not determine UI order. Provide paths.xgb_weekly_output_csv or ranked CSVs in config.")
        return
    ...
    xgb_df = build_outcomes_table(day_labels, daily_dfs, ui_order_xgb, pred_col='pred_xgb')
    st.dataframe(style_outcomes_table(xgb_df), use_container_width=True)

    # Ensemble table (read from config-driven CSV if present)
    st.markdown('**Ensemble: Predicted% / Actual% by Day**')
    ens_map = _load_ensemble_pred_map(_CFG)
    # Inject ensemble predictions into daily_dfs for display under a unified column name
    ens_col = 'ensemble_pred'
    daily_dfs_ens = []
    for d in daily_dfs:
        d2 = d.copy()
        if ens_map is not None:
            d2[ens_col] = d2['ticker'].map(lambda t: ens_map.get(str(t).upper()) if 'ticker' in d2.columns else None)
        else:
            # Fall back to existing snapshot ensemble_score if present
            if 'ensemble_score' in d2.columns:
                d2[ens_col] = pd.to_numeric(d2['ensemble_score'], errors='coerce')
            elif 'pred_lstm' in d2.columns:
                d2[ens_col] = pd.to_numeric(d2['pred_lstm'], errors='coerce')
            else:
                d2[ens_col] = np.nan
        daily_dfs_ens.append(d2)
    ens_df = build_outcomes_table(day_labels, daily_dfs_ens, ui_order_ens, pred_col=ens_col)
    st.dataframe(style_outcomes_table(ens_df), use_container_width=True)

    # Optional: Matplotlib heatmap (actual returns sign)
    try:
        sign_matrix = []
        for tkr in ui_order_xgb:
            signs = []
            for d in daily_dfs:
        ...
        fig_height = max(4, min(12, 0.3 * max(1, len(ui_order_xgb))))
        ...
        ax.set_yticks(range(len(ui_order_xgb)))
        ax.set_yticklabels(ui_order_xgb, fontsize=8)
...
def render_red_tickers_tab(snapshot_dir: str) -> None:
    ...
    price_path = (
        os.getenv('STOCK_PRICES_CSV')
        or (_CFG.get('paths', {}).get('prices_csv') if isinstance(_CFG, dict) else None)
        or (_CFG.get('paths', {}).get('clean_prices') if isinstance(_CFG, dict) else None)
        or 'data/input/stock_prices_with_clean_data.csv'
    )
    if not os.path.exists(price_path):
        st.warning(f'{price_path} not found.')
        return
...
def render_data_ops_tab(current_snapshot_dir: str) -> None:
    ...
    prices_path = st.text_input('Prices CSV path', value=prices_default)
...
def main() -> None:
    ...
    # Snapshot picker
    base_snap = os.getenv('SNAPSHOT_ROOT_DIR', 'snapshot')
    latest = find_latest_snapshot_dir(base_snap)
    snap_dirs = sorted([d for d in glob.glob(os.path.join(base_snap, '*')) if os.path.isdir(d)])
