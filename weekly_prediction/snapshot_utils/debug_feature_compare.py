...
Usage:
  python scripts/debug_feature_compare.py \
...
def main() -> None:
    ...
    print(f"VolRatio {args.ticker_a.upper()} = {float(a_feat['volume_ratio'].iloc[-1]):.3f} (vol_ma_20 computed from last 20 days starting {str(getattr(a_vol_start,'date',a_vol_start))})")
    print(f"VolRatio {args.ticker_b.UPPER()} = {float(b_feat['volume_ratio'].iloc[-1]):.3f} (vol_ma_20 computed from last 20 days starting {str(getattr(b_vol_start,'date',b_vol_start))})")
