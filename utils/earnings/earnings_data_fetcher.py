...
class EarningsDataFetcher:
    ...
    def fetch_earnings_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            earnings_query = """
            SELECT DISTINCT e.ticker AS symbol, e.earnings_date,
    ...
    def _add_volatility_analysis(self, earnings_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"[_add_volatility_analysis] Retrieved {len(price_data)} price data points for {len(price_data['symbol'].unique())} symbols")
            ...
            # Log summary
            logger.info(f"[_add_volatility_analysis] Successfully calculated metrics for {len(symbols_with_data)} symbols")
    ...
    def _clean_earnings_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"[_clean_earnings_data] Cleaning {len(data)} earnings records")
    ...
    def _clean_price_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"[_clean_price_data] Cleaning {len(data)} price records")
            ...
            logger.info(f"[_clean_price_data] Cleaned data: {len(data)} records remaining")
            return data
