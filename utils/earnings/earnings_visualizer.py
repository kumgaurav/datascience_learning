...
class EarningsVisualizer:
    ...
    def __init__(self):
        ...
        logger.info("[EarningsVisualizer] Initialized")
    ...
    def create_earnings_price_charts(self, price_data: pd.DataFrame, selected_symbols: List[str], 
        ...
        try:
            ...
            logger.info(f"[create_earnings_price_charts] Creating individual charts for {len(selected_symbols)} symbols")
            ...
            if earnings_data is not None and not earnings_data.empty and 'total_return' in earnings_data.columns:
                ...
                logger.info("[create_earnings_price_charts] Using pre-calculated returns for sorting")
                logger.info(f"[create_earnings_price_charts] Earnings data contains {len(earnings_data)} records")
                logger.info(f"[create_earnings_price_charts] Earnings data columns: {list(earnings_data.columns)}")
                
                # Show sample of earnings data
                if len(earnings_data) > 0:
                    logger.info("[create_earnings_price_charts] Sample earnings data:")
                    for idx, row in earnings_data.head(3).iterrows():
                        if 'symbol' in row and 'total_return' in row:
                            logger.info(f"[create_earnings_price_charts] Sample: {row['symbol']}: {row['total_return']:.2f}%")
    ...
    def create_volatility_distribution_chart(self, earnings_data: pd.DataFrame) -> None:
        ...
        try:
            ...
            logger.info(f"[create_volatility_distribution_chart] Creating volatility chart for {len(earnings_data)} stocks")
    ...
    def create_earnings_calendar_chart(self, categorized_earnings: Dict[str, pd.DataFrame]) -> None:
        ...
        """
        try:
            logger.info("[create_earnings_calendar_chart] Creating earnings calendar chart")
    ...
    def _sort_symbols_by_precalculated_returns(self, symbols: List[str], earnings_data: pd.DataFrame) -> List[str]:
        ...
        """
        try:
            logger.info(f"[_sort_symbols_by_precalculated_returns] Starting sort for {len(symbols)} symbols: {symbols}")
            ...
            if 'total_return' not in earnings_data.columns:
                ...
                logger.info(f"[_sort_symbols_by_precalculated_returns] Available columns: {list(earnings_data.columns)}")
                return symbols
            
            # Debug: Show earnings data for all symbols
            logger.info(f"[_sort_symbols_by_precalculated_returns] Earnings data shape: {earnings_data.shape}")
            for symbol in symbols:
                ...
                if not symbol_earnings.empty:
                    ...
                    logger.info(f"[_sort_symbols_by_precalculated_returns] {symbol}: total_return = {total_return}")
                else:
            ...
            for symbol in symbols:
                ...
                if not symbol_earnings.empty and 'total_return' in symbol_earnings.columns:
                    ...
                    if pd.notna(total_return):
                        ...
                        logger.info(f"[_sort_symbols_by_precalculated_returns] Added {symbol} with return {total_return:.2f}%")
                    else:
            ...
            logger.info(f"[_sort_symbols_by_precalculated_returns] SORTED ORDER:")
            for i, (symbol, return_val) in enumerate(symbol_returns, 1):
                logger.info(f"[_sort_symbols_by_precalculated_returns] {i}. {symbol}: {return_val:.2f}%")
            
            sorted_symbols = [symbol for symbol, _ in symbol_returns]
            logger.info(f"[_sort_symbols_by_precalculated_returns] Final sorted symbols: {sorted_symbols}")
    ...
    def _create_individual_earnings_charts(self, price_data: pd.DataFrame, symbols: List[str], 
        ...
        try:
            ...
            logger.info(f"[_create_individual_earnings_charts] Creating {len(symbols)} individual charts")
            ...
            logger.info(f"Creating individual earnings charts: {n_rows}x{chart_columns}, "
    ...
    def create_performance_comparison_chart(self, categorized_earnings: Dict[str, pd.DataFrame], 
        ...
        """
        try:
            logger.info("[create_performance_comparison_chart] Creating performance comparison chart")
