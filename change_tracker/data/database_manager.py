...
class DatabaseManager:
    ...
    def _get_engine(self):
        """Get SQLAlchemy engine for SQL queries"""
        if self._engine is not None:
            return self._engine
        try:
            db_config = self.config['mysql']
            url = db_config.get('url', 'localhost')
            port = db_config.get('port', fallback='3306')
            database = db_config.get('database', 'stocksdb')
            username = db_config.get('username', 'root')
            password = db_config.get('password', '')
            charset = db_config.get('charset', 'utf8mb4')
            connection_string = f"mysql+pymysql://{username}:{password}@{url}:{port}/{database}?charset={charset}"
            self._engine = create_engine(connection_string, pool_pre_ping=True, echo=False)
            return self._engine
        except Exception as e:
    ...
    def execute_query(self, query: str, params: Optional[Dict] = None) -> pd.DataFrame:
        ...
        try:
            ...
            # Ensure params is a tuple (single param set) or dict; avoid list which implies executemany
            if isinstance(params, list):
                safe_params = tuple(params)
            else:
                safe_params = params
            df = self._fetch_arrow_safe_df(query, params=safe_params, debug=True)
    ...
    def get_stock_change_tracker(self, page: int = 1, page_size: int = 50, 
        ...
        try:
            ...
            base_query = """
                ...
                LEFT JOIN stocksdb.stocks_earnings se ON sct.symbol  = se.ticker
    ...
    def _fetch_arrow_safe_df(self, sql: str, params: Optional[Dict] = None, debug: bool = True) -> pd.DataFrame:
        """
        Run SQL and return a DataFrame sanitized for Arrow serialization.
        Ensures all columns are Arrow-compatible by constructing a PyArrow Table
        with explicit per-column types, then converts back to pandas.
        Logs dtypes before and after coercion and Arrow schema when debug is True.
        """
        engine = self._get_engine()
