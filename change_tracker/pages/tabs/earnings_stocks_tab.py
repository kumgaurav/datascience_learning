...
class EarningsStocksTab:
    ...
    def _get_earnings_data_with_utilities(self, pre_calculated_data: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        try:
            ...
            if pre_calculated_data is not None and not pre_calculated_data.empty:
                logger.debug(f"[get_earnings_data] Using pre-calculated data with {len(pre_calculated_data)} records")
                return pre_calculated_data
            
            # Fetch fresh data using data fetcher utility
            logger.debug("[get_earnings_data] Fetching fresh earnings data")
    ...
    def _calculate_earnings_metrics(self, earnings_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.debug(f"[calculate_earnings_metrics] Processing {len(earnings_data)} earnings records")
            ...
            if has_volatility_metrics:
                ...
                logger.debug("[calculate_earnings_metrics] Using pre-calculated volatility metrics from data fetcher")
                ...
                logger.debug(f"[calculate_earnings_metrics] Found {non_zero_returns} symbols with non-zero returns, {non_zero_volatility} with non-zero volatility")
                return earnings_data
    ...
    def _display_symbol_selection_and_charts(self, week_data: pd.DataFrame, week_title: str,
        ...
        try:
            ...
            logger.debug(f"[_display_symbol_selection_and_charts] Processing {len(available_symbols)} symbols for {week_title}")
            
            # Sort symbols by returns (highest first) and select top 25 as default
            if 'total_return' in week_data.columns:
                logger.debug(f"[_display_symbol_selection_and_charts] Sorting {len(available_symbols)} symbols by returns")
                ...
                logger.debug("[_display_symbol_selection_and_charts] Top performers by return:")
                for i, (_, row) in enumerate(sorted_week_data.head(10).iterrows(), 1):
                    logger.debug(f"[_display_symbol_selection_and_charts] {i}. {row['symbol']}: {row['total_return']:.2f}%")
                ...
                logger.debug(f"[_display_symbol_selection_and_charts] Selected top {len(default_symbols)} performers as default")
            ...
            logger.debug(f"[_display_symbol_selection_and_charts] About to create charts for {len(selected_symbols)} symbols")
            logger.debug(f"[_display_symbol_selection_and_charts] Selected symbols: {selected_symbols}")
            logger.debug(f"[_display_symbol_selection_and_charts] Week data shape: {week_data.shape}")
            
            if 'total_return' in week_data.columns:
                logger.debug("[_display_symbol_selection_and_charts] Week data contains total_return column")
                ...
                for symbol in selected_symbols[:5]:  # Show first 5 for debugging
                    ...
                    if not symbol_data.empty:
                        ...
                        logger.debug(f"[_display_symbol_selection_and_charts] {symbol} total_return: {return_val:.2f}%")
                    else:
                ...
                logger.debug(f"[_display_symbol_selection_and_charts] Available columns: {list(week_data.columns)}")
