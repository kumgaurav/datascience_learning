...
with col2:
    if st.button("🚀 Find Top Stocks for This Week", type="primary", use_container_width=True):
        with st.spinner("Running advanced analysis... This may take a moment."):
            ...
            # Prefer prebuilt UI dataset if available (strict ensemble-ordered)
            try:
                import pandas as _pd, os as _os
                def _rd(p):
                    try:
                        return _pd.read_csv(p) if _os.path.exists(p) else None
                    except Exception:
                        return None
                _ui = _rd('data/top/ui_dataset.csv')
                if _ui is not None and 'ticker' in _ui.columns:
                    _ui['ticker'] = _ui['ticker'].astype(str).str.upper().str.strip()
                    st.session_state.top_stocks_df = _ui.copy()
                    try:
                        st.caption('Using prebuilt UI dataset (data/top/ui_dataset.csv)')
                    except Exception:
                        pass
            except Exception:
                pass
...
if not st.session_state.top_stocks_df.empty:
        ...
        if selected_ticker:
            ...
            try:
                ...
                if os.path.exists(weekly_path):
                    ...
                    step_ctx = "load_featured_fallback"
    ...
    with tab1:
        ...
        if y_col is not None and 'close' in _chart_df.columns:
            try:
