...
def main() -> None:
    ...
    try:
        ...
        print(f"[DAILY_EVAL] Failed to import XGB feature builder: {e}")
    ...
    # 6) Compute features using feature_engineering utility
    if fe is None or not hasattr(fe, "_calculate_momentum"):
        raise RuntimeError("feature_engineering._calculate_momentum not available to compute features")
    ...
    if args.predict_xgb and joblib is not None:
        try:
            ...
            xgb_feats = [c for c in xgb_feats if c in latest_xgb.columns]
            X_latest = latest_xgb[xgb_feats].replace([np.inf, -np.inf], np.nan).fillna(0)
