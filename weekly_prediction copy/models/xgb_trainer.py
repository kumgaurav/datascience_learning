...
class XGBTrainer(BaseModelTrainer):
    ...
    def _clip_features_by_quantile(self, df_train: pd.DataFrame, df_list: list[pd.DataFrame], feature_cols: list[str], q_low: float = 1.0, q_high: float = 99.0):
        ...
        for c in feature_cols:
            try:
                ...
                # Require minimal sample size to compute stable quantiles
                if finite_vals.size < 10:
                    continue
                lo, hi = np.nanpercentile(finite_vals, [q_low, q_high])
                if not np.isfinite(lo) or not np.isfinite(hi) or lo >= hi:
                    # Fallback to mean ± 3*std clipping if quantiles are degenerate
                    m = float(np.nanmean(finite_vals))
                    s = float(np.nanstd(finite_vals))
    ...
    def _hyperparam_search(self, X_train, y_train, train_group, X_val, y_val, val_group, rng, val_df_local: pd.DataFrame, train_df_local: pd.DataFrame, n_trials: int = 0):
        ...
        def _pick_objective(train_group_sizes):
            # Auto-pick objective if requested; else return env/default
            obj = os.environ.get('XGB_OBJECTIVE', 'auto')
            if obj != 'auto':
                return obj
            # Align with primary tuner metric if specified
            primary_metric = os.environ.get('XGB_TUNER_METRIC', 'p_at_k')
            if isinstance(primary_metric, str) and primary_metric.lower() == 'ndcg_at_k':
                return 'rank:ndcg'
            try:
                avg_group = float(np.mean(train_group_sizes)) if len(train_group_sizes) else 0.0
            except Exception:
                avg_group = 0.0
            # Heuristic: use rank:ndcg when average group is reasonably large
            return 'rank:ndcg' if avg_group >= 20 else 'rank:pairwise'
        ...
        def fit_eval(params):
            primary_metric = os.environ.get('XGB_TUNER_METRIC', 'p_at_k').lower()
            objective_to_use = _pick_objective(train_group)
            eval_metric = None
            if primary_metric == 'ndcg_at_k':
                # NDCG@K evaluation metric (use K=20 matching scoring)
                eval_metric = 'ndcg@20'
            # Ensure ndcg_exp_gain disabled to avoid relevance degree limit
            extra_params = {}
            if isinstance(objective_to_use, str) and objective_to_use.startswith('rank:ndcg'):
                extra_params['ndcg_exp_gain'] = False
            model = xgb.XGBRanker(
            ...
            # Prepare labels: when using rank:ndcg, labels must be non-negative integers
            def _labels_for_objective(y_arr, groups):
                if isinstance(y_arr, (pd.Series, pd.DataFrame)):
                    y_vec = np.asarray(y_arr).ravel()
                else:
                    y_vec = np.asarray(y_arr).ravel()
                if isinstance(objective_to_use, str) and objective_to_use.startswith('rank:ndcg'):
                    out = np.zeros_like(y_vec, dtype=int)
                    idx = 0
                    for gsz in groups:
                        gsz_int = int(gsz)
                        if gsz_int <= 0:
                            continue
                        seg = y_vec[idx:idx+gsz_int]
                        # Higher relevance for higher y; map to integers [0..gsz-1]
                        order = np.argsort(-seg, kind='mergesort')
                        rel = np.empty(gsz_int, dtype=int)
                        rel[order] = np.arange(gsz_int-1, -1, -1)
                        out[idx:idx+gsz_int] = rel
                        idx += gsz_int
                    return out
                # pairwise: can use continuous labels
                return y_vec

            y_train_fit = _labels_for_objective(y_train, train_group)
            y_val_fit = _labels_for_objective(y_val, val_group) if len(X_val) > 0 and len(val_group) > 0 else y_train_fit
            if len(X_val) > 0 and len(val_group) > 0:
                # Compatibility-first fit: avoid callbacks/early stopping for older versions
                try:
                    if eval_metric is not None:
                        model.fit(
                            X_train, y_train_fit, group=train_group,
                            eval_set=[(X_val, y_val_fit)], eval_group=[val_group],
                            verbose=False, eval_metric=eval_metric,
                        )
                    else:
                        model.fit(
                            X_train, y_train_fit, group=train_group,
                ...
                model.fit(X_train, y_train_fit, group=train_group, verbose=False)

            if len(X_val) > 0:
                val_pred = model.predict(X_val)
                metric = os.environ.get('XGB_TUNER_METRIC', 'p_at_k')
                if metric == 'ndcg_at_k':
                    p20 = _ndcg_at_k_by_date(val_df_local, val_pred, k=20, gain=os.environ.get('NDCG_GAIN', 'linear'))
                elif metric == 'mae':
                    try:
                        p20 = -float(np.nanmean(np.abs(val_df_local.get('target_rank', val_df_local['target']).to_numpy() - val_pred)))
                    except Exception:
                        p20 = float('nan')
                else:
                ...
                metric = os.environ.get('XGB_TUNER_METRIC', 'p_at_k')
                if metric == 'ndcg_at_k':
                    p20 = _ndcg_at_k_by_date(train_df_local, val_pred, k=20, gain=os.environ.get('NDCG_GAIN', 'linear'))
                elif metric == 'mae':
                    try:
                        p20 = -float(np.nanmean(np.abs(train_df_local.get('target_rank', train_df_local['target']).to_numpy() - val_pred)))
                    except Exception:
                        p20 = float('nan')
                else:
        ...
        if use_optuna:
            ...
            # direction based on our score definition (we minimize negative of metric)
    ...
    def train(self):
        ...
        # Target stabilization: configurable clipping or log-like transform
        target_mode = os.environ.get('XGB_TARGET_MODE', 'clip')  # 'clip' | 'log1p' | 'none'
        try:
            train_df = train_df.copy(); test_df = test_df.copy()
            if target_mode == 'log1p':
                # Preserve sign, compress extremes
                def _signed_log1p(x):
                    x = pd.to_numeric(x, errors='coerce')
                    return np.sign(x) * np.log1p(np.abs(x))
                train_df['target'] = _signed_log1p(train_df['target'])
                test_df['target'] = _signed_log1p(test_df['target'])
                print("[XGB] Applied signed log1p to target.")
            elif target_mode == 'clip':
                lower, upper = np.percentile(pd.to_numeric(train_df['target'], errors='coerce').dropna(), [1, 99])
                ...
                print("[XGB] Target left untransformed (mode=none).")
        except Exception as _e:
            print(f"[XGB WARN] Target transform failed: {_e}")
        ...
        # --- Optional: penalty/reward rules per request ---
        try:
            import os as _os
            # Penalty: if current day shows selloff but forward target is bullish, downweight
            sell_col = 'selloff_flag'
            if sell_col not in inner_train_df.columns:
                inner_train_df[sell_col] = 0
            selloff = pd.to_numeric(inner_train_df[sell_col], errors='coerce').fillna(0.0) >= 1.0
            # Prefer engineered price_change_pct; fallback to return_1d
            if 'price_change_pct' in inner_train_df.columns:
                pchg = pd.to_numeric(inner_train_df['price_change_pct'], errors='coerce').fillna(0.0)
            else:
                pchg = pd.to_numeric(inner_train_df.get('return_1d', 0.0), errors='coerce').fillna(0.0)
            rsi = pd.to_numeric(inner_train_df.get('rsi_14d', 50.0), errors='coerce').fillna(50.0)
            # Conditions
            bullish_target = pd.to_numeric(inner_train_df.get('target', 0.0), errors='coerce').fillna(0.0) > 0.0
            oversold_bounce = (rsi < 40.0) & (pchg > 0.0)
            # Factors (tunable via env)
            pen_factor = float(_os.getenv('SELLOFF_TRAIN_WEIGHT_FACTOR', '0.70'))  # 30% penalty
            reward_factor = float(_os.getenv('OVERSOLD_BOUNCE_WEIGHT_FACTOR', '1.20'))  # 20% boost
            # Apply
            mask_pen = selloff & bullish_target
            if np.any(mask_pen):
                sw[mask_pen.to_numpy()] = sw[mask_pen.to_numpy()] * pen_factor
            if np.any(oversold_bounce):
                sw[oversold_bounce.to_numpy()] = sw[oversold_bounce.to_numpy()] * reward_factor
            # Clamp
            sw = np.clip(sw, 0.3, 4.0)
        except Exception:
            pass
        ...
        model = best['model']
        # Refit tuned model on full inner-train with sample weights (including penalty/reward)
        try:
            def _labels_for_objective_refit(y_arr, groups, objective: str):
                y_vec = np.asarray(y_arr).ravel()
                if isinstance(objective, str) and objective.startswith('rank:ndcg'):
                    out = np.zeros_like(y_vec, dtype=int)
                    idx = 0
                    for gsz in groups:
                        gsz_int = int(gsz)
            ...
            obj_used = None
            try:
                obj_used = model.get_xgb_params().get('objective', None)
            except Exception:
                obj_used = None
            y_train_fit_refit = _labels_for_objective_refit(y_train, train_group, str(obj_used or 'rank:pairwise'))
            try:
                model.fit(
                    X_train, y_train_fit_refit,
                    group=train_group,
                    sample_weight=sw,
                    verbose=False,
                )
            except Exception:
                # Fallback without sample weights if unsupported by version
                model.fit(X_train, y_train_fit_refit, group=train_group, verbose=False)
        except Exception:
            pass
        try:
        ...
        # Feature importance logging/export
        try:
            importances = None
            if hasattr(model, 'feature_importances_'):
                ...
                try:
                    ...
                    fmap = {i: f for i, f in enumerate(self.feature_cols)}
                    score = booster.get_score(importance_type=os.environ.get('XGB_IMPORTANCE_TYPE', 'gain'))
                    # Map to DataFrame with all features
                    importances = np.array([score.get(f, 0.0) for f in self.feature_cols], dtype=float)
                except Exception:
                    importances = None
            if importances is not None:
                ...
                imp_path = os.path.join('metrics', f'feature_importances_{ts_name}.csv')
                imp_df.to_csv(imp_path, index=False)
                print(f"[XGB] Wrote feature importances to {imp_path}")
                if getattr(self, 'tracker', None):
                    try:
                        self.tracker.log_artifact(imp_path, artifact_path='metrics')
                    except Exception:
                        pass
        except Exception as _e:
            print(f"[XGB WARN] Failed to export feature importances: {_e}")
