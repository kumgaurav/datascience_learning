...
class BaseModelTrainer:
    ...
    def prepare_data(self, price_col='close', horizon=5, feature_engineering_mode: str = 'internal', shared_feature_config: dict | None = None):
        df = self.df.copy()

        # Validate price column
        if price_col not in df.columns:
            alt_col = 'close' if 'close' in df.columns else None
            if alt_col is None:
                raise ValueError(f"price_col '{price_col}' not found and no 'close' column present in features CSV")
            try:
                print(f"[BASE PREP] price_col '{price_col}' missing; falling back to '{alt_col}'")
            except Exception:
                pass
            price_col = alt_col
        ...
        try:
            ...
            raise ValueError(f"Failed to compute target using price_col '{price_col}': {_e}")
        ...
        # Train/test split (time-based) using sorted unique dates for robust cutoff
        df = df.sort_values(['ticker', 'date'])
        try:
            uniq_dates = pd.Series(pd.to_datetime(df['date'], errors='coerce').unique()).dropna().sort_values().to_list()
            if not uniq_dates:
                raise ValueError('No valid dates found for split')
            cutoff_idx = max(int(len(uniq_dates) * 0.8) - 1, 0)
            cutoff = uniq_dates[cutoff_idx]
            try:
                print(f"[BASE PREP] Split cutoff at {cutoff} using unique-date 80th percentile (n_dates={len(uniq_dates)})")
            except Exception:
                pass
        except Exception:
            # Fallback to quantile on timestamp
    ...
    def build_time_series_cv_splits(self, n_splits: int = 5) -> list[tuple[pd.DataFrame, pd.DataFrame]]:
        """Build expanding-window time-series CV splits based on unique dates.

        Returns a list of (train_df, val_df) pairs with non-overlapping validation windows.
        """
        if not hasattr(self, 'df') or self.df is None or len(self.df) == 0:
            raise ValueError('Base trainer has no data loaded to build CV splits')
        df = self.df.copy()
        df['date'] = pd.to_datetime(df['date'], errors='coerce')
        df = df.dropna(subset=['date']).sort_values(['ticker', 'date'])
        uniq_dates = pd.Series(df['date'].unique()).sort_values().to_list()
        if len(uniq_dates) < (n_splits + 1):
            # Not enough unique dates; fallback to single split 80/20
            cutoff_idx = max(int(len(uniq_dates) * 0.8) - 1, 0)
            cutoff = uniq_dates[cutoff_idx] if uniq_dates else df['date'].max()
            return [(df[df['date'] <= cutoff], df[df['date'] > cutoff])]
        # Determine fold boundaries using time-delta based windows to handle gaps
        ...
        try:
            ...
            for i in range(1, n_splits + 1):
                # Fractional boundaries across the time span
                train_frac = i / (n_splits + 1)
                val_frac = (i + 1) / (n_splits + 1)
                train_end_date = date_min + total_span * train_frac
                val_end_date = date_min + total_span * val_frac
                train_df = df[df['date'] <= train_end_date]
                val_mask = (df['date'] > train_end_date) & (df['date'] <= val_end_date)
                val_df = df[val_mask]
                if len(val_df) == 0:
                    continue
                splits.append((train_df.copy(), val_df.copy()))
        except Exception:
            # Fallback to index-based splitting on unique dates
            fold_size = max(len(uniq_dates) // (n_splits + 1), 1)
            for i in range(1, n_splits + 1):
                train_end_idx = i * fold_size
                val_end_idx = min((i + 1) * fold_size, len(uniq_dates))
                train_end_date = uniq_dates[train_end_idx - 1]
                val_end_date = uniq_dates[val_end_idx - 1]
                train_df = df[df['date'] <= train_end_date]
                val_mask = (df['date'] > train_end_date) & (df['date'] <= val_end_date)
                val_df = df[val_mask]
                if len(val_df) == 0:
        ...
        try:
            print(f"[BASE CV] Built {len(splits)} time-series CV splits using time-based windows (total_dates={len(uniq_dates)})")
        except Exception:
            pass
        return splits
